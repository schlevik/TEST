...
from datasets_classification.dataset import ImputationDataset, TransductionDataset, ClassiregressionDataset, collate_unsuperv, collate_superv
...
def fit_encoder_classifier_parameters(text_prototype, dataset_x, dataset_labels, cuda, gpu,local_rank,
                        save_memory=False):
    """
    ...
    """
    classifier = wrapper.CausalCNNEncoderClassifier()
    ...
    params['reduced_size'] = text_prototype.size(1)
    classifier.set_params(**params)
    return classifier.fit(
        dataset_x, dataset_labels, save_memory=save_memory, verbose=True
    )


def fit_encoder_parameters(text_prototype, dataset_x, cuda, gpu,local_rank,
                        save_memory=False):
    """
    @param file Path of a file containing a set of hyperparemeters.
    @param train Training set.
    ...
    return encoder.fit_encoder(
        dataset_x, save_memory=save_memory, verbose=True
    )